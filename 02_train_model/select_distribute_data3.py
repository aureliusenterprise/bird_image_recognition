...
df = pd.read_csv(base_dir+"output/file_metadata2_non.csv", sep=";")

df.columns
df.dtypes

df = df.iloc[1:2000].copy()
df['Bird'].fillna(0, inplace=True)
...
df['Train'] = df[df['Bird']==0]['number']%5>0
df['Val'] = df[df['Bird']==0]['number']%5==0

# detected as not birds, but are not birds
...
for f in files:
    try:
        if ii % 3 ==0:
            shutil.move(base_dir+"images/non/"+f, base_dir+"dataset/train/not_bird/a_"+f)
        ii+=1
    except:
...
for f in files:
    try:
        if ii % 3 ==0:
            shutil.move(base_dir+"images/non/"+f, base_dir+"dataset/val/not_bird/a_"+f)
        ii+=1
    except:
        print(f"failed for {f}")
...
for f in files:
    shutil.move(base_dir+"images/non/"+f, base_dir+"dataset/train/bird/a_"+f)
